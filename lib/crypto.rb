...
def crypto_name
  ...
  return crypto_name_array = page.xpath('//td[@class = "cmc-table__cell cmc-table__cell--sortable cmc-table__cell--left cmc-table__cell--sort-by__symbol"]/div').map {|node| node.text}
end
...
def crypto_price
  ...
  return crypto_price_array = page.xpath('//div[@class = "price___3rj7O "]/a[@class = "cmc-link"]').map {|node| node.text.gsub(/[^\d\.]/, '').to_f}
end
